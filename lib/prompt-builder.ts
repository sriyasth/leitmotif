...
function inferInstrument(character: string): string {
  ...
  if (c.includes('warm') || c.includes('gentle') || c.includes('soft') || c.includes('round')) return 'celesta'
  if (c.includes('sharp') || c.includes('angular') || c.includes('focused') || c.includes('bright')) return 'solo muted trumpet'
  if (c.includes('playful') || c.includes('young') || c.includes('energetic') || c.includes('quick')) return 'kalimba'
  if (c.includes('calm') || c.includes('reserved') || c.includes('elegant')) return 'solo clarinet'
  if (c.includes('grounded') || c.includes('steady') || c.includes('earthy')) return 'marimba'
  return 'glockenspiel'
}
...
export function buildLyriaPrompt(input: PromptBuilderInput): string {
  ...
  if (!previousState) {
    lines.push(`Generate an extremely minimal ambient background.`, `Scene vibe: ${vibe}. Environment: ${environment}.`, ...bgRules, `Tempo: 70-90 bpm.`)
  } else if (vibeChanged) {
    lines.push(`Scene mood shifting from "${previousState.vibe}" to "${vibe}".`, `Slowly crossfade the single background pad to match the new vibe.`, ...bgRules)
  } else if (hasMotifChange) {
    lines.push(`Scene vibe remains "${vibe}". Keep the background pad unchanged.`, ...bgRules)
  } else {
    lines.push(`Continue the same quiet background pad. Do not change anything.`, ...bgRules)
  }
  ...
  if (motifs.length > 0) {
    ...
    motifs.forEach((m, i) => {
      ...
      lines.push(`\nMotif ${label} (${m.name}):`, `${frozenDescription}`)
